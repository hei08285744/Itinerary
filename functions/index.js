...
exports.socialShare = onRequest({ region: REGION, cors: false }, async (request, response) => {
  ...
  const imageUrl = ticket ? `${HOSTING_ORIGIN}/ticket-preview/${encodeURIComponent(tripId)}.png?renderer=stamp-5&${previewQuery}` : `${HOSTING_ORIGIN}/assets/mytinerary-trip-ticket-preview.png?v=3`;
  const appUrl = `${HOSTING_ORIGIN}/?trip=${encodeURIComponent(tripId || '')}`;
  response.set('Cache-Control', 'public, max-age=300, s-maxage=300');
